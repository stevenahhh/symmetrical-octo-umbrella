...
function createTimetable(specialType, index) {
  ...
  floorDefs.forEach(({ floor, rows }) => {
    rows.forEach((cells, rowIndex) => {
      ...
      cells.forEach((cell) => {
        ...
        rooms.push({
          id: isUtility ? `d4-${wingId}-utility-${floor}-${rowIndex}-${offset}` : `d4-${wingId}-${cell.number}`,
          ...
          timetable: createTimetable(specialType, floor * 20 + index),
        });
        offset += cell.width;
      });
    });
