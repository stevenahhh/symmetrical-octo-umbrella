...
export function BuildingSectionView({ building, rooms, roomUsageById, selectedRoomId, onSelectRoom }) {
  const { camera } = useThree();
  const floors = useMemo(() => [...(building?.floors ?? [])].sort((a, b) => a.floor - b.floor), [building?.floors]);
      ...
      <Html position={[-5.6, 6.6, -2.6]} transform distanceFactor={14}>
        <div className="w-[280px] rounded-lg border border-white/20 bg-black/55 px-4 py-3 text-white shadow-xl backdrop-blur-md">
          ...
          <div className="mt-1 text-[12px] font-[600] leading-5 text-white/70">
            왼쪽 동 5층, 오른쪽 동 6층, 3·5층 연결 다리를 반영한 목업입니다.
          </div>
          <div className="mt-3 flex items-center gap-3 text-[11px] font-[800] text-white/75">
            <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm bg-[#3b82f6]" />사용 중</span>
            <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm bg-[#94a3b8]" />미사용</span>
          </div>
        </div>
      </Html>
        ...
        return (
          <group key={floor.id} position={[0, y, 0]}>
            ...
            {hasBridge && (
              <group>
                ...
                <Html position={[0, 0.32, 0]} center distanceFactor={10}>
                  <div className="rounded-md border border-sky-100/70 bg-sky-950/80 px-2 py-1 text-[11px] font-[900] text-white shadow">연결 다리</div>
                </Html>
              </group>
            )}
            ...
            {floorRooms.map((room) => {
              ...
              const color = isInUse ? ACTIVE_ROOM_COLOR : IDLE_ROOM_COLOR;

              return (
                <group key={room.id} position={[room.geometry.x, height / 2, room.geometry.z]} onPointerUp={(event) => {
                  event.stopPropagation();
                  onSelectRoom(room.id);
                }}>
                  <mesh castShadow receiveShadow>
                    <boxGeometry args={[room.geometry.width, height, room.geometry.depth]} />
                    <meshStandardMaterial color={color} emissive={selected ? "#f59e0b" : "#000000"} emissiveIntensity={selected ? 0.18 : 0} />
                  </mesh>
