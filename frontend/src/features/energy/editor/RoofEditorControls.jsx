import { List, Plus, Redo2, RotateCcw, Save, Trash2 } from "lucide-react";
...
export function RoofEditorControls({ buildingId = "D4", arrays = [], selectedArray, summary, canSave, canUndo, status, violations, onAdd, onSelect, onUpdate, onDelete, onUndo, onSave, onReload, onBackToPlans }) {
  return <aside className="relative z-20 max-h-[44vh] shrink-0 overflow-y-auto border-t border-[var(--colors-hairline-strong)] bg-[color-mix(in_srgb,var(--colors-surface-1)_96%,transparent)] p-4 shadow-2xl backdrop-blur-xl md:h-full md:max-h-none md:w-[360px] md:border-l md:border-t-0 xl:w-[380px]" aria-label="태양광 설치 제어판">
    <div className="flex items-start justify-between gap-3">
      ...
      <div className="flex shrink-0 flex-col items-stretch gap-2">
        ...
        {onBackToPlans && <button className={buttonClass} type="button" aria-label="설치 계획 목록으로 돌아가기" onClick={onBackToPlans}><List size={15} /> 계획 목록</button>}
      </div>
    </div>
